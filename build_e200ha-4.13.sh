...
if [ ! -d "sound-topic/asus-e100h-4.13" ]; then
	if [ ! -f "asus-e100h-4.13.tar.gz" ]; then
		wget https://git.kernel.org/pub/scm/linux/kernel/git/tiwai/sound.git/snapshot/sound-topic/asus-e100h-4.13.tar.gz
	fi
	tar xvf asus-e100h-4.13.tar.gz
	cd sound-topic/asus-e100h-4.13/
	make clean && make mrproper
	cp config-e200ha sound-topic/asus-e100h-4.13/.config
	make xconfig
	make
else
	cd sound-topic/asus-e100h-4.13/
